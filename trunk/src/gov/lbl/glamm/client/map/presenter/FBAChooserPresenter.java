...
public class FBAChooserPresenter {
	...
	private static enum State {
		...
		NO_FBA("No FBA results found in this workspace"),
	...
	public FBAChooserPresenter(final GlammServiceAsync rpc, final View view, final SimpleEventBus eventBus) {
		this.rpc = rpc;
	...
	private void bindView() {
		...
		view.getLoadSubsetButton().addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (!dataList.isEmpty())
					eventBus.fireEvent(new FBAChooserEvent(FBAChooserEvent.Action.SELECT, dataList));
				clear();
				view.getPopupPanel().hide();
			}
		});
	...
	private void populateWorkspaceTable() {
		workspaceDataProvider.getList().clear();
		setViewState(State.POPULATING_FBA);

		String workspace = view.getWorkspaceListBox().getItemText(view.getWorkspaceListBox().getSelectedIndex());
		rpc.populateWorkspaceFbas(workspace, new AsyncCallback<List<KBWorkspaceObjectData>>() {

			@Override
			public void onFailure(Throwable caught) {
				Window.alert("Remote procedure call failure: populateWorkspaceFbas.\nUnable to load model information from workspace!");
				setViewState(State.NO_FBA);
			}
