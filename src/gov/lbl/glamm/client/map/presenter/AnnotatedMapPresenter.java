...
import gov.lbl.glamm.client.map.events.MapElementMouseOutEvent;
import gov.lbl.glamm.client.map.events.MapElementMouseOverEvent;
	...
	public void updateMapForRoute(final Compound cpdSrc, final Compound cpdDst, final Pathway route) {
		...
		if (route != null) {
			for(Reaction reaction : route.getReactions()) {
				for(OMSVGElement element : mapData.getSvgElements(reaction)) 
					element.setAttribute(AnnotatedMapData.Attribute.ROUTE, reaction.getColor().getCssAttributeValue());
			}
